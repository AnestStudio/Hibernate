...
Ngoài các tham số được đặt tên có dạng `?index` thì chúng ta cũng có thể sử dụng các liên kết tham số được đặt tên, chỉ cần xác định tên cho từng tham số và cung cấp tên đó cho phương thức `setParameter(String var1, Object var2)` để liên kết một giá trị với tham số đó. Tên có phân biệt chữ hoa chữ thường và bạn cần thêm ký hiệu `:` làm tiền tố. Ví dụ `:name`, `:id`...
...
Ví dụ tiếp theo, chúng ta sẽ thay đổi truy vấn đã sử dụng trước đó và đổi tên cột:
- `user_id => id`
- `username => account`
