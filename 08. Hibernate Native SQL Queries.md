...
Cách dễ nhất để ánh xạ kết quả truy vấn tới một entity là cung cấp lớp entity làm tham số cho phương thức `createNativeQuery(String sqlString, Class resultClass)` của __EntityManager__ và sử dụng ánh xạ mặc định. Các bạn có thể xem lại phần này ở mục [2.1. The entity mapping]().
