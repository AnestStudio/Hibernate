...
Từ Hibernate 5 và JPA 2.2, chúng ta có thể lặp lại `@NamedNativeQuery()` annotation để đặt tên cho nhiều câu native query.
```java
@NamedNativeQuery(
    name="FindUsers",
    query="SELECT * FROM [user]"
...
Sau đó chúng ta cần sử dụng phương thức `setParameter()` để gán giá trị cho các tham số trong câu query.
```java
setParameter(int var1, Object var2)
```
