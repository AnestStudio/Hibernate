...
Chúng ta có thể sử dụng 2 phương thức `Session.get()` và `Session.find()` để truy xuất một thực thể từ cơ sở dữ liệu dựa trên khóa chính của nó.

| Tính năng |	`Session.get()` | `Session.find()` |
| --------- | ----------------- | ---------------- |
| Đặc tả	  | Hibernate       |	JPA            |
| Class 	  | Session.get()   | EntityManager.find() |
| Kết quả	  | Trả về __null__ nếu không tìm thấy đối tượng |	Trả về __null__ nếu không tìm thấy đối tượng |

Một điểm khác biệt nữa về mặt ngữ nghĩa sử dụng: `find()` có thể có kết quả hoặc không, `get()` sẽ luôn trả về một vài thứ gì đó thậm chí là __null__.
...
- `Session.update()`: nó là một original Hibernate, không còn được sử dụng nữa từ version 6.0.
- `Session.merge()`: phương thức này nằm trong đặc tả JPA
  ...
  - Nếu đối tượng __chưa được quản lý__ (detached state): Hibernate sẽ sao chép trạng thái của đối tượng vào một đối tượng mới được quản lý bởi session. Sau đó, đối tượng mới này sẽ được so sánh với dữ liệu trong database và cập nhật tương tự như
...
- `Session.delete()`: nó là một original Hibernate, không còn được sử dụng nữa từ version 6.0.
