...
Trong mối liên kết hai chiều, một trong các bên (và chỉ một) phải là chủ sở hữu. Chủ sở hữu chịu trách nhiệm cập nhật (các) cột liên kết. Để tuyên bố bên nào không chịu trách nhiệm về mối quan hệ, thuộc tính `mappedBy` được sử dụng. `mappedBy` liên kết đến tên của thuộc tính liên kết về phía chủ sở hữu. Như trong ví dụ `mappedBy = "user"` liên kết đến thuộc tính `user` trong class `UserDetail`.
