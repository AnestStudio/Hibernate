...
| ------ | -------------- | ------------- |
| cascade      | - | {} |
| fetch        | - | __LAZY__ cho `@OneToMany` và `@ManyToMany`, __EAGER__ cho `@ManyToOne` |
| targetEntity | Lớp thực thể liên quan | Xác định từ khai báo kiểu thuộc tính |
| optional     | Đối với liên kết `@ManyToOne` hoặc `@OneToOne`, liên kết đó có thể `null` hay không | `true` |
